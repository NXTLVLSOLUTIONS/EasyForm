...
static CGFloat const kMenuTopSpacing = 1.f;
...
- (instancetype)init
{
	if (self = [super init]) {
		...
		[NSLayoutConstraint activateConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(<=1)-[_menuTableView]-(<=1)-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_menuTableView)]];
		...
		self.defaultTitleAttributes = @{NSForegroundColorAttributeName: [UIColor whiteColor], NSFontAttributeName: [UIFont boldSystemFontOfSize:17.f]};
		...
		self.defaultSelectedMenuItemAttributes = @{NSForegroundColorAttributeName: EASY_BLUE, NSFontAttributeName: [UIFont systemFontOfSize:16.f]};
	}
	return self;
}
...
- (void)installNavigationTitleView
{
	...
	if (!attributedTitle && [self.dataSource respondsToSelector:@selector(navigationMenuItem:menuTitleAtIndex:)]) {
   	...
   	attributedTitle = [[NSAttributedString alloc] initWithString:@"Select Category" attributes:self.defaultTitleAttributes];
	}
	
	self.navigationTitle = [[UILabel alloc] initWithFrame:CGRectMake(30.0, 0.0, CGRectGetWidth([UIScreen mainScreen].bounds), 44.0)];
...
-(void) refreshNavTitle: (NSString*) title{
    
    NSAttributedString *attributedTitle = [[NSAttributedString alloc] initWithString:title attributes:self.defaultTitleAttributes];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([ParseDataFormatter sharedInstance].isSelectionMade == YES) {
...
//		attributedMenuTitle = [[NSAttributedString alloc] initWithString:[self.dataSource navigationMenuItem:self menuTitleAtIndex:indexPath.row] ?: @"" attributes:cell.isSelected ? self.defaultSelectedMenuItemAttributes : self.defaultMenuItemAttributes];
        ...
        attributedMenuTitle = [[NSAttributedString alloc] initWithString:[self.dataSource navigationMenuItem:self menuTitleAtIndex:indexPath.row] attributes:self.defaultMenuItemAttributes];
	}
