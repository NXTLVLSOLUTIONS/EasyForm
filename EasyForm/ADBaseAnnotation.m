...
- (id)initWithParseObject:(PFObject*)object locationName: (NSString*) subtitle{
    
    PFGeoPoint *mapPoint = object[@"coordinates"];
    NSString   *nameString  = [NSString stringWithFormat:@"%@ %@",object[@"firstName"], object[@"lastName"]];
  
    return [self initWithCoordinates:CLLocationCoordinate2DMake(mapPoint.latitude, mapPoint.longitude)
                               title:nameString
                            subtitle:subtitle];
}
