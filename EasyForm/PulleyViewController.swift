...
open class PulleyViewController: UIViewController, UIScrollViewDelegate, PulleyPassthroughScrollViewDelegate {
    ...
    override open func viewDidLoad() {
        ...
        self.navigationItem.title = "EASYFORM"
        
        let button = UIButton.init(type: .custom)
        button.setImage(UIImage.init(named: "threeline"), for: UIControlState.normal)
        button.addTarget(self, action:#selector(self.menuButtonPressed), for: UIControlEvents.touchUpInside)
        button.frame = CGRect.init(x: 0, y: 0, width: 25, height: 20) //CGRectMake(0, 0, 30, 30)
